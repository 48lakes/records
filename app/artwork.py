...
async def enrich_with_artwork(rec: Dict[str, Any]) -> Dict[str, Any]:
    """Best-effort enrichment: find MB RG, fetch cover, write full (600px) and 150px thumb.
    Returns updated record dict including 'mb_release_group_id', 'cover_art_url', 'cover_thumb_url'.
    """
    ensure_dirs()
    ...
    # Fallback to Discogs cover image URL if MB not available
    if not img_bytes:
